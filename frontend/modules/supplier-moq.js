...
function showMoqFormLoading(show) {
  const form = document.getElementById("moqForm");
  if (!form) return;
  
  let loader = document.getElementById("moqFormLoading");
  
  if (show) {
    if (!loader) {
      loader = document.createElement("div");
      loader.id = "moqFormLoading";
...
function renderMoqRegionDropdown() {
  ...
  el.innerHTML = `
    <label class="flex items-center gap-2 font-semibold border-b mb-1 pb-1">
      <input type="checkbox" class="moq-select-all" data-target="moqRegionDropdown" value="">
      ทั้งหมด
    </label>
  ` + regions.map(r => `
...
function renderMoqBranchDropdown() {
  ...
  el.innerHTML = `
    <label class="flex items-center gap-2 font-semibold border-b mb-1 pb-1">
      <input type="checkbox" class="moq-select-all" data-target="moqBranchDropdown" value="">
      ทั้งหมด
    </label>
  ` + filtered.map(b => `
    <label class="flex items-center gap-2">
...
/* ===============================
 ...
 ================================ */
async function initProductFilter(prefix, supplierNo) {

  await loadCoverageToForm(supplierNo, {
    category: `${prefix}Cat`,
    brand: `${prefix}Brand`,
    group: `${prefix}Group`,
    sub: `${prefix}Sub`,
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Show form loading
  showMoqFormLoading(true);

  console.log("🔥 MOQ supplierNo =", supplierNo);

  // 🔥 ใช้ shared logic
  await initProductFilter("moq", supplierNo);
