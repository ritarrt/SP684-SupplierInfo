...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (catSelect && brandSelect) {
    catSelect.addEventListener("change", () => {
      loadGroups(catSelect.value, brandSelect.value, "dealGroup");
  ...
  if (btn) {
    btn.addEventListener("click", async () => {
      ...
      const region = getSelectedValues("dealRegionDropdown").join(",");
      const province = getSelectedValues("dealProvinceDropdown").join(",");
      const branch = getSelectedValues("dealBranchDropdown").join(",");
      ...
      try {
        ...
        if (isEditMode && dealId) {
          ...
          res = await fetch(
            `${API_BASE}/api/suppliers/${supplierNo}/deals/${dealId}`,
            {
              method: "PUT",
  ...
  async function updateDealStatus(status) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/suppliers/${supplierNo}/deals/${currentToggleDealId}/status`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        // If error is about changing dates, show message and open edit modal
        if (errorMessage.includes('กรุณาเปลี่ยนวันที่เริ่มใช้ราคาและวันสิ้นสุดก่อนเปิดใช้งานดีลอีกครั้ง')) {
          showSaveMessage(errorMessage, true);
          // Close modal and open edit modal
          modal.classList.add("hidden");
          modal.classList.remove("flex");
          openEditDealModal(currentToggleDealId);
          return;
        }
...
window.renderDealTable = function() {
  ...
  rows.forEach((r, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <span
          class="badge ${r.status === "OPEN" ? "bg-success" : r.status === "USE" ? "bg-primary" : "bg-danger"}"
      ...
      <td>
        ${(() => {
          ...
          const canEdit = 
            ...
            (r.status === 'CANCELLED' && (isExpired || r.has_been_used)) ||
            r.has_been_used; // Can edit if deal has been used before
...
function openDealModal(dealId, status, hasBeenUsed = false) {
  ...
  if (status === "OPEN") {
    ...
    btnUse?.classList.add("hidden");
    // Allow reopening cancelled deals even if they were used before
    if (hasBeenUsed) {
      message.innerHTML =
        "ดีลราคาถูกยกเลิกแล้ว<br>⚠️ ดีลนี้เคยใช้งานมาก่อน กรุณาแก้ไขวันที่เริ่มใช้ราคาและวันสิ้นสุดก่อนเปิดใช้งาน";
    } else {
      message.innerHTML =
        "ดีลราคาถูกยกเลิกแล้ว<br>ต้องการเปลี่ยนสถานะหรือไม่?";
    }
  }
...
function openEditDealModal(dealId) {
  ...
  // Only allow editing if status is OPEN or (CANCELLED and (expired or has been used))
  if (deal.status === 'USE') {
    showSaveMessage("ไม่สามารถแก้ไขได้ เนื่องจากดีลกำลังถูกใช้งาน", true);
    return;
  }

  if (deal.status === 'CANCELLED' && !isExpired && !deal.has_been_used) {
    showSaveMessage("ไม่สามารถแก้ไขได้ เนื่องจากดีลถูกยกเลิกแล้ว", true);
    return;
  }

  // Show warning for deals that have been used before
  if (deal.has_been_used) {
    showSaveMessage("⚠️ กรุณาเปลี่ยนทั้งวันที่เริ่มใช้ราคาและวันสิ้นสุด", false);
  }
