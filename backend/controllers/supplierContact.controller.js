...
// ===============================
// PATCH /api/suppliers/contacts/:id/reactivate
// ===============================
export async function reactivateSupplierContact(req, res) {
  try {
    const { id } = req.params;
    if (!id) return res.status(400).json({ message: "contact id is required" });
    ...
    console.error("reactivateSupplierContact error:", err);
    res.status(500).json({ message: "reactivate contact failed" });
  }
}
